<template>
    <div class="*:cursor-pointer">
        <button @click="emit('page-changed', currentPage - 1)" :disabled="currentPage === 1"
            class="px-3 py-1 rounded-md mr-2 bg-white hover:bg-gray-100 disabled:opacity-50">
            Anterior
        </button>
        ...
        <button @click="emit('page-changed', currentPage + 1)" :disabled="currentPage === totalPages"
            class="px-3 py-1 rounded-md ml-1 bg-white hover:bg-gray-100 disabled:opacity-50">
            Próximo
        </button>
    </div>
</template>
